...
export const QUEST_TEMPLATES: QuestTemplate[] = [
  ...
  // bronze; collapsed to a single entry-level tier).
  {
    ...
    tier: "bronze",
    applies: () => true,
    render: m => `Walk the grounds of ${m.name} on foot and capture a photo from inside.`,
    verify: "photo",
  ...
  },
  {
    ...
    render: m => `Walk the lawn or plaza around ${m.name} for 30+ min — circle it at least twice.`,
    verify: "duration",
    companionBumpEligible: true,
  },
  ...
  // Aurora (legendary) — physical altitude or arrival challenge.
  {
    ...
    render: m => m.isTower
      ? `Climb the stairs (no elevator) to the highest accessible deck of ${m.name}.`
      : `Hike to the iconic high vantage point at ${m.name} on foot.`,
    verify: "altitude",
    companionBumpEligible: true,
  },
  ...
  // signature, the off-the-trail viewpoint.
  {
    ...
    render: m => `Walk off the main path and explore until you find a lesser-known viewpoint of ${m.name}.`,
