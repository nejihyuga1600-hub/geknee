...
const TRANSPORT_ICONS: Record<string, string> = {
  flight: "✈️", train: "🚄", bus: "🚌", ferry: "⛴️", subway: "🚇",
};
...
function ImageSlideshow({
  ...
  if (loading) return (
    <div style={{ ...baseStyle, background: "linear-gradient(90deg,#f1f5f9 25%,#e2e8f0 50%,#f1f5f9 75%)", backgroundSize: "200% 100%", animation: "shimmer 1.4s infinite" }} />
  );
...
function Skeleton({ h = 200 }: { h?: number }) {
  return <div style={{ height: h, borderRadius: 14, background: "linear-gradient(90deg,#f1f5f9 25%,#e2e8f0 50%,#f1f5f9 75%)", backgroundSize: "200% 100%", animation: "shimmer 1.4s infinite" }} />;
}
...
// Each function receives (originIata, destIata, depDate YYYY-MM-DD, retDate YYYY-MM-DD)
// and returns a pre-filled booking URL for that airline.
type AirlineLinkFn = (o: string, d: string, dep: string, ret: string) => string;

const AIRLINE_BOOKING: Record<string, AirlineLinkFn> = {
  // ── North America ──────────────────────────────────────────────────────────
  // AA: deep-link via Kayak (AA's own SPA ignores query params on cold load)
  AA: (o,d,dep,ret) =>
    `https://www.kayak.com/flights/${o}-${d}/${dep}/${ret}?cabin=e&adults=1`,
  // UA: pre-filled search via united.com book widget
  UA: (o,d,dep,ret) =>
    `https://www.united.com/en/us/fsr/choose-flights?f=${o}&t=${d}&d=${dep}&r=${ret}&tt=2&sc=7&px=1&taxng=1&newHP=True`,
  // DL: Delta pre-fill via hash fragment (works in their SPA router)
  DL: (o,d,dep,ret) =>
  ...
  // NH: ANA deep-link (format verified from ANA booking widget docs)
  NH: (o,d,dep,ret) =>
...
/** Build a pre-filled airline booking URL, or fall back to the AI-generated URL */
...
function FlightSection({ location, startDate, endDate, travelingFrom }: {
  ...
  // Resolve destination: try full name, then first word (handles "Tokyo Skytree" → "Tokyo" → NRT)
  ...
  return (
    <section style={{ marginBottom: 44 }}>
      ...
      {/* Flight price chart — same component as style page */}
...
// Official + Klook links where online purchase is actually possible for tourists.
// Key = lowercase city name (partial match).  Values shown in order on the card.
...
// Find transit links for a city — partial case-insensitive match
    ...
    <div style={{
      ...
      {loading && !text ? (
        ...
        <p style={{ margin: "0 0 10px", fontSize: 12, color: "#475569", lineHeight: 1.65, whiteSpace: "pre-wrap" }}>
          {text}
          {loading && <span style={{ animation: "blink 0.9s step-end infinite", marginLeft: 2 }}>|</span>}
        </p>
      )}
      {(() => {
...
function LocalTransitSection({ legs, cities, budget, startDate }: {
  ...
  // Parse a human-readable or ISO date string into URL-ready formats
  ...
  // Format trip startDate (YYYY-MM-DD) into FlixBus DD.MM.YYYY
  const [sdY, sdM, sdD] = (startDate || "").split("-");
  const tripFlixDate = sdY && sdM && sdD ? `${sdD}.${sdM}.${sdY}` : "";

  return (
    <section style={{ marginBottom: 44 }}>
      <SectionHeader icon={String.fromCodePoint(0x1F68C)} title="Bus &amp; Subway" />

      {/* Inter-city bus / subway connections */}
        ...
        <div style={{ marginBottom: 22 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 12 }}>
            {busSubwayLegs.map((leg, i) => {
              ...
              const flixDate = legDate?.flix ?? tripFlixDate;
      ...
      {/* Subway pass recommendations per destination city */}
      <p style={{ margin: "0 0 12px", fontWeight: 700, fontSize: 13, color: "#1e293b" }}>
...
function BookContent() {
  ...
  // ── Trip Setup panel state (mirrors style page) ───────────────────────────
  const today = new Date().toISOString().split("T")[0];

  const [fromAirport,  setFromAirport]  = useState(() => airportLabel(travelingFrom));
  ...
  const destIata   = useMemo(() => extractIata(toAirport),   [toAirport]);
  ...
  function regenItinerary() {
    const q = new URLSearchParams({
      location, purpose, style: style, budget, interests,
      startDate: pendingStart, endDate: pendingEnd,
      nights: String(Math.round((new Date(pendingEnd).getTime() - new Date(pendingStart).getTime()) / 86400000)),
    });
    if (travelingFrom) q.set("travelingFrom", travelingFrom);
    if (stopsRaw) q.set("stops", stopsRaw);
  ...
  useEffect(() => {
    const itinerary = typeof window !== "undefined" ? sessionStorage.getItem("geknee_itinerary") ?? "" : "";
    fetch("/api/recommendations", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ location, purpose, style, budget, interests, startDate, endDate, nights, itinerary }),
    })
      .then(r => r.json())
  ...
  }, [location, purpose, style, budget, interests, startDate, endDate, nights]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#f8fafc 0%,#ede9fe 100%)", fontFamily: "'Segoe UI',system-ui,sans-serif" }}>
      <style>{`
      ...
      {/* Header */}
      <div style={{ background: "linear-gradient(135deg,#6366f1,#8b5cf6,#a78bfa)", padding: "28px 24px 24px", color: "#fff" }}>
        <div style={{ maxWidth: 1720, margin: "0 auto" }}>
          <p style={{ margin: "0 0 4px", fontSize: 11, opacity: .75, letterSpacing: 1, textTransform: "uppercase" }}>GeKnee · Book your trip</p>
          <h1 style={{ margin: "0 0 5px", fontSize: 28, fontWeight: 800 }}>
            {isMultiStop ? allStops.map(s => s.city).join(" → ") : location}
          </h1>
          <p style={{ margin: 0, opacity: .8, fontSize: 13 }}>
            {nights} nights · {startDate} to {extraStops.length > 0 ? extraStops[extraStops.length - 1].endDate : endDate} · {style} · {budget}
          </p>
        </div>
      </div>

      {/* ── Trip Setup panel ── */}
      <div style={{ background: "linear-gradient(135deg,#f8fafc 0%,#ede9fe 100%)", borderBottom: "1px solid #e2e8f0" }}>
        <div style={{ maxWidth: 1720, margin: "0 auto", padding: "28px 24px 24px" }}>
          <style>{`
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
              <span className="ts-label" style={{ margin: 0 }}>Traveling from</span>
              {(travelingFrom || geoStatus === "done") && (
                <span style={{ display: "flex", alignItems: "center", gap: 5, background: "rgba(129,140,248,0.1)", border: "1px solid rgba(129,140,248,0.3)", borderRadius: 999, padding: "3px 12px", fontSize: 12, fontWeight: 600, color: "#6366f1" }}>
                  {String.fromCodePoint(0x1F4CD)} {findAirport(travelingFrom || fromAirport)?.city ?? travelingFrom}
                </span>
              )}
            </div>
            <div style={{ position: "relative" }}>
              <input className="ts-input" value={fromAirport}
              ...
              {geoStatus === "loading" && (
                <span style={{ position: "absolute", right: 12, top: "50%", width: 16, height: 16, borderRadius: "50%", border: "2px solid rgba(99,102,241,0.25)", borderTopColor: "#6366f1", animation: "spin 0.8s linear infinite", display: "inline-block" }} />
              )}
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
              <span className="ts-label" style={{ margin: 0 }}>Traveling to</span>
              {location && (
                <span style={{ display: "flex", alignItems: "center", gap: 5, background: "rgba(129,140,248,0.1)", border: "1px solid rgba(129,140,248,0.3)", borderRadius: 999, padding: "3px 12px", fontSize: 12, fontWeight: 600, color: "#6366f1" }}>
            ...
            <div style={{ position: "relative" }}>
              <input className="ts-input" value={toAirport} onChange={e => setToAirport(e.target.value)} placeholder="Arrival airport…" style={{ paddingRight: toAirport ? 40 : undefined }} />
            ...
            {toAirport && <p style={{ fontSize: 11, color: "#94a3b8", margin: "5px 0 0" }}>Nearest commercial airport — edit if needed</p>}
          </div>

          {/* Travel dates */}
          <div>
            <p className="ts-label">Travel dates</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div>
                <label style={{ fontSize: 11, color: "#94a3b8", display: "block", marginBottom: 5 }}>Departure</label>
                <input type="date" className="ts-input" min={today} value={tripStartDate}
                  onChange={e => handleDateChange("start", e.target.value)} />
              </div>
              <div>
                <label style={{ fontSize: 11, color: "#94a3b8", display: "block", marginBottom: 5 }}>Return</label>
                <input type="date" className="ts-input" min={tripStartDate || today} value={tripEndDate}
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.65)", backdropFilter: "blur(4px)" }}>
          <div style={{ background: "#0f172a", border: "1px solid rgba(251,191,36,0.35)", borderRadius: 20, padding: "32px 28px", maxWidth: 440, width: "90%", boxShadow: "0 24px 64px rgba(0,0,0,0.7)" }}>
            ...
            <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 13, textAlign: "center", lineHeight: 1.6, margin: "0 0 24px" }}>
              Some date-specific activities in your itinerary may need to be updated. Would you like to re-generate your itinerary with the new dates?
            </p>
      ...
      <div style={{ maxWidth: 1720, margin: "0 auto", padding: "32px 20px 80px" }}>
        ...
        {isMultiStop && (
          <section style={{ marginBottom: 40 }}>
            <SectionHeader icon="🗺️" title="Your Route" loading={legsLoading} />
            {legsLoading ? (
              <Skeleton h={280} />
            ) : (
              <>
        ...
        {/* Hotels */}
        <section style={{ marginBottom: 44 }}>
          <SectionHeader icon="🏨" title="Hotels" loading={recsLoading} count={!recsLoading ? recommendations?.hotels.length : undefined} />
          {recsLoading ? (
        ...
        {/* Restaurants */}
        <section style={{ marginBottom: 44 }}>
          <SectionHeader icon="🍽️" title="Restaurants" loading={recsLoading} count={!recsLoading ? recommendations?.restaurants.length : undefined} />
          {recsLoading ? (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(290px,1fr))", gap: 16 }}>
        ...
        {/* Activities */}
        <section style={{ marginBottom: 44 }}>
          <SectionHeader icon="🎯" title="Activities & Experiences" loading={recsLoading} count={!recsLoading ? recommendations?.activities.length : undefined} />
          {recsLoading ? (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(290px,1fr))", gap: 16 }}>
              <Skeleton h={340} /><Skeleton h={340} /><Skeleton h={340} />
            </div>
  ...
  return (
    <Suspense fallback={
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f8fafc" }}>
        <p style={{ color: "#6366f1", fontSize: 18 }}>Loading your trip…</p>
      </div>
    }>
      <BookContent />
    </Suspense>
